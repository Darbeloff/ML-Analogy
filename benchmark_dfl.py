...
def randu(m,n,a,b):
	return (b-a)*np.random.rand(m,n)+a
...
if __name__ == '__main__':
	...
	# Parameters
	k, b, m, dt, T = 1, 2, 1, 0.1, 100
	M = 10000
	H, l_x, l_eta, l_u = 256, 2, 2, 1

	# Hand-crafted eta
	def g(x):
		Fs = k*x[0,:][None,:]**3
		Fd = b*x[1,:][None,:]**3
		return np.concatenate((Fs, Fd), 0)
	...
	# Compute H
	xi = np.concatenate((x_tm1, eta_tm1, u_tm1), 0)
	# H = torch.matmul(eta_t, torch.pinverse(xi))
	H = np.transpose(np.linalg.lstsq(np.transpose(xi), np.transpose(eta_t), rcond=None)[0])
	# breakpoint()
	# A, H, _, _ = DFL(x_t, x_tm1, g, u_tm1)

	# Simulate step response
	x0 = np.array([[0],[0]])
	x   = float('nan')*np.ones((2,T))
	xs  = float('nan')*np.ones((2,T))
	eta = float('nan')*np.ones((2,T))
	u   =          0.5*np.ones((1,T))
	x  [:,0] = np.squeeze(  x0 )
	xs [:,0] = np.squeeze(  x0 )
	eta[:,0] = np.squeeze(g(x0))
	for t in range(1,T):
		x[:,t] = f(x[:,t-1][:,None], u[:,t-1][:,None]).squeeze()
		xi_tm1 = np.concatenate((xs[:,t-1], eta[:,t-1], u[:,t-1]), 0)[:,None]
		xs[:,t] = f(xs[:,t-1][:,None], u[:,t-1][:,None], eta[:,t-1][:,None]).squeeze()
		eta[:,t] = np.matmul(H,xi_tm1).squeeze()
		# breakpoint()
	# Illustrate
	plt.figure()
	# plt.plot(range(T), u [0,:], label='u')
	plt.plot(range(T), xs[0,:], label='xs')
	plt.plot(range(T), x [0,:], label='x')
	# plt.ylim(-0.5,1)
	plt.xlabel('Time')
	plt.legend()
	plt.show()