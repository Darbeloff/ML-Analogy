...
def f(x,y):
	out = np.sin(y)*np.exp((1-np.cos(x))**2)+np.cos(x)*np.exp((1-np.sin(y))**2)+(x-y)**2
	if (x+5)**2+(y+5)**2>=25:
		out = float('nan')
	return out
x_min, x_max, y_min, y_max = -10,0,-6.5,0
density = 50
x0, y0 = -9.5,-4.5
...
# torch.save(model.state_dict(), 'mishra.pt')
...
model.load_state_dict(torch.load('mishra.pt'))
