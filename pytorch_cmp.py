...
if __name__ == '__main__':
	...
	def f(x,u,eta=None):
		...
		return torch.matmul(torch.from_numpy(A_x  ).type(dtype),   x) + \
			   torch.matmul(torch.from_numpy(A_eta).type(dtype), eta) + \
			   torch.matmul(torch.from_numpy(B_x  ).type(dtype),   u)

	# Create training data
	x_tm1 = randu(2,M,-3,3).type(dtype)
	...
	# Compute H
	xi = torch.cat((x_tm1, eta_tm1, u_tm1), 0)
	# H = torch.matmul(eta_t, torch.pinverse(xi))
	H = torch.transpose(torch.lstsq(torch.transpose(eta_t,0,1), torch.transpose(xi,0,1)).solution[:5,:] ,0,1)
	# breakpoint()
