...
def f(x,y):
	out = 0.1*x*y
	rT, rS, n = 1, 0.2, 8
	if x**2+y**2>(rT+rS*np.cos(n*np.arctan(x/y)))**2:
		out = float('nan')
	return out
x_min, x_max, y_min, y_max = -1.25,1.25,-1.25,1.25
density = 100
x0, y0 = 0.5,0.5
...
# torch.save(model.state_dict(), 'simionescu.pt')
...
model.load_state_dict(torch.load('simionescu.pt'))
